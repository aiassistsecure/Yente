...
// The tick lives in src/runtime/desk.js so that bin/yente.mjs — which runs the
// desk beside the graph loops in one process — executes the same code rather
// than a copy of it. Behaviour is unchanged; only its address is.
const desk = createDesk({ store, runtime, log, mode: "daemon" });
const tick = () => desk.tick();
