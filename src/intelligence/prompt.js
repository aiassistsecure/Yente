...
export const OBSERVER_SYSTEM = [
  ...
  "<<<END>>> on its own line. Reply with exactly one OBSERVATIONS block and",
  "nothing else: no prose before or after it, and no markdown code fence. Never",
  "emit a delimiter inside a block's content.",
  "",
...
const OUTPUT_CONTRACT = [
  "Return exactly one OBSERVATIONS block whose content is a single JSON object",
  "with these keys, each an array, each omittable when empty:",
  "",
  ...
  "                   `ref` is a short label you invent (e.g. \"p1\", \"org2\") and",
  "                   reuse below to refer to this entity. `kind` must be one of",
  "                   the entity kinds given in CONTROLLED_VOCABULARY.",
  "",
  "  intents        — what an entity is seeking, offering, hiring for, raising,",
  "                   buying, selling, and so on.",
  "                   { actor_ref, type, object, attributes?,",
  "                     source_id, evidence, explicit, confidence }",
  "                   `object` is what the intent is ABOUT, in a few words.",
  "                   `attributes` may carry specifics you can quote, such as",
  "                   skills, sector, stage, amount, timing, or location.",
  "",
  ...
  "",
  "  observations   — anything else worth remembering about a person or",
  "                   organisation that the above shapes do not fit.",
  "                   { subject_ref?, text, source_id, evidence, explicit, confidence }",
  "",
  "RULES.",
  "  `ref` values must be declared in `entities` before use. A claim referring to",
  "  an undeclared ref is discarded.",
  "  `source_id` must be the id of the SOURCE block you took the quote from.",
  ...
  "  Report nothing you cannot quote. An empty array is a good answer.",
].join("\n");
